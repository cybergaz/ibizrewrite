...
const ProductList = ({ products }: { products: Items }) => {
    ...
    return (
        <div className="flex flex-col gap-5 items-center">
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-y-10 gap-x-4 sm:gap-x-6 md:gap-x-8 lg:gap-x-12">
                {
                    products?.map((product, index) => (
                        <div
                            key={index}
                            className="w-full max-w-[15rem] h-[18rem] p-3 flex flex-col justify-between animate-fade-in-scale delay-500 shadow-2xl gap-2 bg-accent-light-gray/40 border border-black/5 rounded-2xl">
